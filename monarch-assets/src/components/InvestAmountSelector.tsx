    ...
    <Collapsible defaultOpen={!defaultCollapsed} className="w-full">
      ...
      <CollapsibleContent className="space-y-3 pt-3">
        <p className="px-1 text-[11px] tabular-nums text-muted-foreground">
          ~{tokensApprox.toFixed(2)} tokens at reference mark ({formatUsd(lotUsd)} / token)
        </p>
        <div className="px-1">
          <Slider
        ...
        <div className="flex justify-between text-[10px] text-muted-foreground">
          <span className="tabular-nums">{formatUsd(minUsd)}</span>
          <span className="tabular-nums">{formatUsd(maxUsd)}</span>
        </div>
        <p className="text-[10px] leading-snug text-muted-foreground">
          At least one token at the reference mark · up to {formatUsd(INVEST_GLOBAL_MAX)} or remaining float (API
