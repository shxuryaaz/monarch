...
router.post("/intent", requireAuth, async (req, res, next) => {
  try {
    ...
    if (tokenAmount > asset.availableSupply + SUPPLY_FLOAT_EPSILON) {
...
router.post("/confirm", requireAuth, async (req, res, next) => {
  try {
    ...
    if (nextSupply < -SUPPLY_FLOAT_EPSILON) {
    ...
    const mintPayload = {
      ...
      assetToken: intent.asset.tokenAddress,
      amount: intent.tokenAmount.toString(),
      to: intent.asset.tokenAddress,
      data: "0x",
    ...
    res.json({ intent: updated, mintPayload });
  } catch (error) {
    next(error);
  }
});
